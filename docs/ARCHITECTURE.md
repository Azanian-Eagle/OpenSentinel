...
- **React SDK (`@opensentinel/react`)**: TypeScript wrapper providing the `useOpenSentinel` hook with automated dependency handling and SSR safety checks.
- **Python SDK (`opensentinel`)**: Lightweight client wrapper relying strictly on Python standard libraries (`urllib`).
- **NPM Vanilla JS (`@opensentinel/client`)**: UMD client asset usable across bundlers or direct CDN script inclusions.
